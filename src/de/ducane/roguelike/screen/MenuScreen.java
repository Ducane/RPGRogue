...
public class MenuScreen extends Screen
{
	...
	private Rectangle2D[]					buttonBounds	= new Rectangle2D[ BUTTON_LABELS.length ];
	...
	public void onResized( final int width, final int height )
	{
		...
		buttonFont = new Font( BUTTON_FONT_NAME, BUTTON_FONT_STYLE, (int) ( BUTTON_FONT_SIZE * getHeight() ) );
		...
		{
			buttonBounds[ i ] = new Rectangle2D.Float( buttonX, buttonY + buttonDy * i, buttonWidth, buttonHeight );
		}
	...
	private void renderBackground( final Graphics2D g )
	{
		...
		g.drawLine( 0, getHeight() - backgroundDecoYOffset, getWidth(), getHeight() - backgroundDecoYOffset );
	}
	...
	private void renderCursor( final Graphics2D g )
	{
		...
		final int[] y =
		{ buttonY + buttonHeight / 5 + buttonDy * selection,
		...
		final int[] newY =
		{ buttonY + (int) ( buttonHeight / 2.5f ) + buttonDy * selection,
				buttonY + buttonHeight - (int) ( buttonHeight / 2.5f ) + buttonDy * selection,
				buttonY + buttonHeight / 2 + buttonDy * selection };
		
		final int[] interpolX =
		{ (int) FloatMathUtil.interpolate( x[ 0 ], cursorProgress, newX[ 0 ] ),
				(int) FloatMathUtil.interpolate( x[ 1 ], cursorProgress, newX[ 1 ] ),
				(int) FloatMathUtil.interpolate( x[ 2 ], cursorProgress, newX[ 2 ] ) };
		
		final int[] interpolY =
		{ (int) FloatMathUtil.interpolate( y[ 0 ], cursorProgress, newY[ 0 ] ),
				(int) FloatMathUtil.interpolate( y[ 1 ], cursorProgress, newY[ 1 ] ),
				(int) FloatMathUtil.interpolate( y[ 2 ], cursorProgress, newY[ 2 ] )
		};
	...
	{
		selection = IntMathUtil.modulo( selection + shift, BUTTON_LABELS.length );
	}
